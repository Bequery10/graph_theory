...
// if (keys[i].compareTo(key) < 0)
// throw new IllegalArgumentException(
// "Calling decreaseKey() with a key strictly greater than the key in the
// priority queue");
// keys[i] = key;
// swim(qp[i]);
// }
...
// if (keys[i].compareTo(key) > 0)
// throw new IllegalArgumentException(
// "Calling increaseKey() with a key strictly less than the key in the priority
// queue");
// keys[i] = key;
// sink(qp[i]);
// }
...
// // validate that pq[i] is a valid index
